...
class Session {
    ...
    async request(options) {
        return new Promise(async resolve => {
            ...
            if (res[2].status == 429 && options.retry !== false) await setTimeout(async () => resolve(await this.request(options)), (parseInt(res[2].headers.get('RateLimit-Reset'))) * 1000)
            else if (res[2].status == 401) this.event('loginfailed')
            else resolve(res)
        })
    }
